...
The wrapper starts alternate-port local servers, uses a temporary data directory, isolates Codex/Claude subscription homes by default, verifies `/health`, `/capabilities`, `/chat/status`, `/provider-config/status` including provider cards and the OpenRouter disabled gate, `/connector-status`, `/guardian/status` including the LIMA provider execution boundary, `/round-table/status`, `/model-seats/status`, `/work-lanes/status`, `/local/chat/sessions`, `/local/memory-notes`, `/local/work-lane-cards`, `/local/export`, `/local/runtime/settings`, `/local-models/status`, disabled-mode `POST /local-models/ollama/prompt` returning 403, frontend HTTP response, temporary data-dir reporting, API-key provider onboarding with placeholder backend keys, OpenRouter guarded-manual status, non-OpenRouter API providers remaining `disabled-by-default`, non-free OpenRouter model rejection returning 400 before provider dispatch, and enabled-mode local model status. It does not send an enabled local-model prompt or any successful cloud prompt by default. Use `SPARKBOT_SMOKE_USE_HOST_SUBSCRIPTIONS=true` only when the test is specifically checking host Codex/Claude sign-in readiness; add `SPARKBOT_SMOKE_REQUIRE_SUBSCRIPTIONS=true` when a LIMA/operator install smoke should fail unless both subscription cards are signed in and still LIMA-gated.
